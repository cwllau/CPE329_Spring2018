...
void keypad_init(void) {
    P4->DIR = 0;
    P4->REN = 0x00;     /* enable pull resistor for column pins */
...
char keypad_getkey(void) {
    ...
    P4->OUT &= 0x0F;           /* drive all row pins high */
    delay();                    /* wait for signals to settle */
    col = P4->IN & 0xF0;        /* read all column pins */
    P4->OUT |= 0x00;            /* drive all rows low before disable them */
    P4->DIR &= ~0x0F;           /* disable all row pins drive */
    if (col == 0x00)            /* if all columns are low */
    ...
    if (col == 0xD0) return row * 3 + 1;    /* key in column 1 */
    if (col == 0xB0) return row * 3 + 2;    /* key in column 2 */
    if (col == 0x70) return row * 3 + 3;    /* key in column 3 */
