...
void keypad_init(void) {
    P4->DIR = 0;
    P4->REN = 0x70;     /* enable pull resistor for column pins */
    P4->OUT = 0x70;     /* make column pins pull-ups */
}
...
char keypad_getkey(void) {
    int row, col;
    const char row_select[] = {0x01, 0x02, 0x04, 0x80}; /* one row is active */

    /* check to see any key pressed */
    P4->DIR |= 0x8F;            /* make all row pins output */
    P4->OUT &= ~0x8F;           /* drive all row pins low */
    delay();                    /* wait for signals to settle */
    col = P4->IN & 0x70;        /* read all column pins */
    P4->OUT |= 0x8F;            /* drive all rows high before disable them */
    P4->DIR &= ~0x8F;           /* disable all row pins drive */
    ...
    for (row = 0; row < 4; row++) {
        ...
        col = P4->IN & 0x70;            /* read all columns */
